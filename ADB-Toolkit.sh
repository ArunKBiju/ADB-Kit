...
check_new_version () {
    ...
    if ping -q -c 1 -W 1 google.com >/dev/null; then
        checked_version=$(curl -s https://raw.githubusercontent.com/ARUNKBIJU/ADB-Toolkit/master/modules/version)
...
if [ $(id -u) -ne 0 ]; then
        echo "THIS SCRIPT MUST BE RUNNING AS A ROOT COMMAND"
        exit 1
fi
...
if [ -d $PWD/.temp/ ]
    then
        ...
        echo -e "PLEASE INSTALL THE TOOL IF NOT INSTALLED, OR IF IT IS INSTALLED MAKE A '.temp' DIRECTORY FIRST" && exit
fi
...
\e[0m"
echo
echo -e "\e[93mMade with ❤️ by \e[1;91mARUN K BIJU\e[0m"
}
bash modules/funtion.sh

#Copyright : github.com/arunkbiju , ARUN K BIJU , arunkbiju.github.io/My-Portfolio
